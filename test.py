...
for dataset in dataset_data:
    ...
    ## 12/10 'active datasetVersion has no harmonized phenotypes, so to test im setting datasetVersion manually to '2'
    dss_dataset_id = 2
    
    ## 12/10 commenting out fileset retrieval because may phase out
    ...
    sample_files_list = []
    non_sample_files_list = []
    allconsents_files_list = []
    ...
    request_url = APIURL+"datasetVersions/"+str(dss_dataset_id)+"/fileSamples"
    ...
    request_url = APIURL+"datasetVersions/"+str(dss_dataset_id)+"/fileNonSamples"
    print('checking to see if there are nonSample files from ' + request_url)
    response = requests.get(request_url, headers=headers)
    if len(response.json()["data"]) > 0:

        request_url = APIURL+"datasetVersions/"+str(dss_dataset_id)+"/fileNonSamples?per_page=1000"
        print( 'getting non-sample files from ' + request_url )
        response = requests.get(request_url, headers=headers)

        last_page = response.json()["meta"]["last_page"]
        file_nonsample_data = response.json()["data"]

        for file in file_nonsample_data:
            non_sample_files_list.append(file)
        ...
        if last_page > 1:
            for page in range( last_page + 1 ):
                if page < 2:
                    ...
                    response = requests.get(request_url + "&page=" + str(page), headers=headers)
                    print('getting paginated data from ' + request_url + "&page=" + str(page))
                    files_non_sample_data = response.json()["data"]
                    for non_sample_file in files_non_sample_data:
                        files_non_sample_files_list.append(non_sample_file)
    else:
    ...
    request_url = APIURL+"datasetVersions/"+str(dss_dataset_id)+"/fileAllConsents"
    print('checking to see if there are nonSample files from ' + request_url)
    response = requests.get(request_url, headers=headers)
    if len(response.json()["data"]) > 0:
            ##tested versioning-compliant url = https://dev3.niagads.org/darm/api/datasetVersions/{id}/fileAllConsents
        request_url = APIURL+"datasetVersions/"+str(dss_dataset_id)+"/fileAllConsents?per_page=1000"
        print( 'getting all-consent files from ' + request_url )
        response = requests.get(request_url, headers=headers)
        last_page = response.json()["meta"]["last_page"]
        file_allconsents_data = response.json()["data"]

        for file in file_allconsents_data:
            allconsents_files_list.append(file)
        ...
        if last_page > 1:
            for page in range( last_page + 1 ):
                if page < 2:
                    ...
                    print('getting paginated data from ' + request_url + "&page=" + str(page))
                    file_allconsents_data = response.json()["data"]
                    for all_consents_file in file_allconsents_data:
                        allconsents_files_list.append(all_consents_file)
    else:
        print('no all-consent files, moving on...')
    
    print(str(len(sample_files_list)) + " sample file(s) retrieved total for dataset " + str(dss_dataset_id))
    print(str(len(non_sample_files_list)) + " nonsample file(s) retrieved total for dataset " + str(dss_dataset_id))
    print(str(len(allconsents_files_list)) + " allconsent file(s) retrieved total for dataset " + str(dss_dataset_id))
    print('-------files loop--------')
    ...
    response = requests.get(APIURL+"datasetVersions/"+str(dss_dataset_id)+"/consents", headers=headers)
    ...
    for c in dataset_consents:
        ...
        if len(project_sample_set) > 0:
            ...
            ## 12/10 commenting out for now because may phase out fileset stuff
            ...
            #     submitter.submit_record(program_name, project_name, fileset_obj)
                ...
                ## first filtering on fileset_id (in sample_files_list object) == fileset_id
                ## then by c (current consent)
            for file in sample_files_list:
                if file["sample"]["subject"]["consent"] is not None:
                    if file["sample"]["subject"]["consent"]["key"] == c:
                ...
                ## first filtering on fileSetId (in non_sample_files_list object) == fileset_id
                ## then by c (current consent) == consent_key in the list
            for file in non_sample_files_list:
                if file["consent"] is not None:

                    if file["consent_key"] == c:
                ...
                # first filtering on fileset_id (in allconsents_files_list object) == fileset_id
            for file in allconsents_files_list:
